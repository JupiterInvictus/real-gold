...
*/
		if (isadmin()){
			...
			$previousmonth = date($format, strtotime('-1 month'));

			if ($_GET['b'] == 'clearprt073') {
				$sql = "DELETE FROM prt073data where month = '$currentmonth' OR month = '$previousmonth'";
				sq($sql);
			}
			if ($_GET['b'] == 'clearprt060') {
				$sql = "DELETE FROM prt060data where month = '$currentmonth' OR month = '$previousmonth'";
				sq($sql);
			}

			echo "<h3>General PRT report uploader.</h3>";
			echo "<a href='?a=settings&b=clearprt073'>Clear PRT073data</a> / ";
			echo "<a href='?a=settings&b=clearprt060'>Clear PRT060data</a>";
