...
set_time_limit(600);
session_start();
$application_version_major = "16.10.05";
$application_version_minor = "14.51";
...
elseif ($validated==''){
		...
		if ($msd[$z][2]=="IDLE_Break"){
			if (substr($msd[$z][3],0,2)=="01"){
				if (substr($msd[$z][3],3,2)>0) {
					...
					<button>Click to appeal</button></span><br><br>\n";
				}
			}
		}
  ...
  echo "<b>Thanks!</b><br>$breaks";
	echo "$unbreak";
	echo "$training";
	if ($training){
		echo "<textarea id=trnmtgexplanation name=trnmtgexplanation cols=90 rows=10></textarea><br>";
	}
