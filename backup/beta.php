...
$application_version_major = "2017-07-04";
$application_version_minor = "08:12";
...
function displayvmbox($dvm_team, $dvm_metric, $dvm_value) {
  ...
  // Target title
  echo "<span class='targettitle' style='color:#$fg'>tgt</span>";
  // Target area
...
function displaydashboardbox($ddb_team, $ddb_metric, $submetric){
  ...
  echo "<span class='sevendaystitle'";
  echo " style='color:#{$fg};'>";
  echo "7d";
  echo "</span>";

  // Last week value area
  echo "<span class='sevendaysarea'";
  ...
  echo "<div class='sevendaysdelta delta";
...
function addleftchoice($name) {
  global $_GET, $showleftchoicetext;
  ...
  if ($showleftchoicetext) {
    ...
    echo "' title='" . ucfirst($name) . "'><img src='images/{$name}.png'>";
  }
  echo "</div>";
}
...
function av($what,$monthdate,$filter){
	...
	$sql = "SELECT phone_rcr,transfer_rate,total_aht_secs,contacts_handled,ntid,queue_name,phone_answered,phone_aht_secs,email_worked,email_aht_secs FROM ahtreport_data WHERE month='$montha-$yeara'$ahtteamdefinition $filter";
	if(!$result=$db->query($sql)){cl($sql);cl($db->error);}
	$contra = 0;
	$counto=0;
	$counta=0;
...
<html>
	<head>
		...
		<link href='https://fonts.googleapis.com/css?family=Lato|Raleway|Raleway:900' rel='stylesheet'>
...
if ($_GET['n']) {
    ...
    if ($showleftchoicetext) {
    	echo "<a href='http://www.concentrix.com' id='cnx_logo'><img src='cnx_logo.png'></a>";
		 	...
		 	echo "<div id='div_username'>$un</div>";
    }
	...
	addleftchoice("contracts");
  	...
  	$sub = $_GET[sub];
		...
		echo "VM055: " . dd($vm055updated) . " / ";
		echo "PRT060p: " . dd($prt060pupdated);
		echo "<br><br>$time</div>";
  	echo "<div id='prevmonth$small'><a href='?a=$a&b=$b&c=$c&d=$d&team=$team&startdate=$lastmonth&enddate=$lastend'><img src='images/rewind-1.png' title='Previous month'></a></div>";
  	echo "<div id='nextmonth$small'><a href='?a=$a&b=$b&c=$c&d=$d&team=$team&startdate=$nextmonth&enddate=$nextend'><img src='images/fast-forward-1.png' title='Next month'></a></div>";
    ...
    if ($a) {
      ...
      echo "<div id='area_body'>";
				...
				if ($team>0){
        			...
        			if(1) {
						...
						<th class=rotated width=10>Email NPS</th>
      					<th class=rotated width=10>FCR</th>
						...
						echo "<td>";
          				...
          				echo "</td>"; // Email NPS
          				$fcrtarget = gettarget(5,16,$team,$view_startdate, "high");
          				echo "<td>$fcrtarget</td>";
						echo "<td>98%</td>";
          				echo "<td></td>";
          				echo "<td></td>";
          				echo "<td></td>";
						...
						for($x=1;$x<=$teammate_counter;$x++){
							if (guessteam($teammate[$x])==$team){
								...
								$bd[emailaht][$teammate[$x]] = av("eaht",$startdate,$ahtdef);
              					...
              					$bd[fcr][$teammate[$x]] = sv(16,$combinedsurveydefinition,$bd[combinedsurveys][$teammate[$x]]);
						...
						foreach($bd[performancepoints] as $tm => $points){
							if (guessteam($tm)!=$team){
								...
								// FCR
              					list($bg,$fg)=targetcolor($bd[fcr][$tm], $contract, 16, $team, $view_startdate);
								echo "<td	style='color:#$fg;background-color:#$bg'>{$bd[fcr][$tm]}</td>";
								// Performance Points
								...
								echo "<td";
              					if (($bd[fcr][$tm]>=$fcrtarget) && ($bonusbeforesick[$tm]>0)){
			...
			if($a=='bonusnew'){
				if ($team>0){
					...
					if(1) {
						...
						<th>Name</th>
						<th>Surname</th>
						<th class=rotated>Contacts</th>
						<th class=rotated>Surveys</th>
						...
						echo "</tr></thead><tfoot>";
						echo "<tr>";
						echo "<td colspan=5><b>TARGET</b></td>";
							...
							for($x=1;$x<=$teammate_counter;$x++){
								if (guessteam($teammate[$x])==$team){
  								...
  								$bd[instances][$teammate[$x]] = getinstances($teammate[$x],$startdate);
									...
									/*$weight_aht = 0; if ($bd[aht][$teammate[$x]]) {$weight_aht = min(1.1, $tgt_aht/$bd[aht][$teammate[$x]]);}
									$weight_kdi = 0; if ($bd[kdi][$teammate[$x]]) {$weight_kdi = min(1, max(0.8, $bd[kdi][$teammate[$x]]/$tgt_kdi));}
									$weight_tr = 0; if ($bd[tr][$teammate[$x]]) {$weight_tr  = min(1, max(0.4, $tgt_tr/$bd[tr][$temmate[$x]]));}
									$weight_rcr = 0; if ($bd[rcr][$teammate[$x]]) { $weight_rcr = min(1, max(0.5, $tgt_rcr/$bd[rcr][$temmate[$x]])); }
									...
									// If AHT is met or lower than good target, scale weight depending on how well the user is performing, maximum 130%.
									if ($bd[aht][$teammate[$x]] <= $tgt_goodaht){
										// Pick whichever is smaller. the maxweight or the target / actual ratio.
										$weight_aht = min($maxweight, $tgt_goodaht/$bd[aht][$teammate[$x]]);
									}
									// if user only meets fair target, award 70% of weighting.
									elseif ($bd[aht][$teammate[$x]] < $tgt_aht) {
										$weight_aht = $goodweight;
									}
									// If target is not met, award nothing.
									else {
										$weight_aht = 0;
									}


								  if ($bd[kdi][$teammate[$x]] >= $tgt_kdi) {
										$weight_kdi = min($maxweight, $bd[kdi][$teammate[$x]]/$tgt_kdi);
									}
								  else {
										$weight_kdi = 0;
									}

									if ($bd[tr][$teammate[$x]]*100 <= $tgt_tr) {
										$weight_tr = min($maxweight, $tgt_tr/($bd[tr][$teammate[$x]]+0.00001)/100);
									}
									else {
										$weight_tr = 0;
									}

									if ($bd[rcr][$teammate[$x]]*100 <= $tgt_rcr) {
										$weight_rcr = min($maxweight, $tgt_rcr/($bd[rcr][$teammate[$x]]+0.000001)/100);
									}
									else {
										$weight_rcr = 0;
									}
                	...
                	//$bd[performancepoints][$teammate[$x]] = $weight_aht * $bonus_weight[2] + $weight_kdi * $bonus_weight[5] + $weight_tr * $bonus_weight[6] + $weight_rcr * $bonus_weight[17];
  						...
  						foreach($bd[performancepoints] as $tm => $points){
  							if (guessteam($tm)!=$team){
  								...
  								echo "<td class=tmname>$surname</td>";
  								// contacts
								echo "<td>{$bd[contacts][$tm]}</td>";

  								// Surveys
  								echo "<td>{$bd[surveys][$tm]}</td>";
                	...
                	echo "</td>";
            					...
            					list($bg,$fg)=targetcolor($bd[performancepoints][$tm], $contract, 11,5);
  								...
  								// Bonus before sick
  								$bonusbeforesick[$tm] = $bd[hours][$tm] * 1.22;
  								if ($bd[performancepoints][$tm]<0.75){$bonusbeforesick[$tm]=0;}
  								elseif ($bd[performancepoints][$tm]<0.9){$bonusbeforesick[$tm]*=0.5;}
									elseif ($bd[performancepoints][$tm]>1.1){$bonusbeforesick[$tm]*=1.1;}
  								echo "<td";
                					if (($bd[nps][$tm]>=$npstarget) && ($bonusbeforesick[$tm]>0)){
                  					$bonusbeforesick[$tm]*=1.2;
                  					echo " style='color:#06f'>";
			...
			elseif($a=='calendar'){
        echo "<h2>PRT060p</h2>";
        $sql = "SELECT DISTINCT month FROM ahtreport_data";
        if(!$result=$db->query($sql)){cl($sql);cl($db->error);}
        while ($row=$result->fetch_assoc()){
      ...
      elseif($a=='masters'){
        ...
        while ($row=$result->fetch_assoc()){
            ...
            if (($team<0) || (guessteam($row[teammate_nt_id])==$team)){
			  ...
			  // Contacts
              $sqlb = "SELECT SUM(Phone_Answered) as pa, SUM(Email_replied) as er FROM ahtreport_data WHERE NTID = '{$row[teammate_nt_id]}' AND Queue_Name <> 'Total'  $ahtdates";
              if(!$resultb=$db->query($sqlb)){cl($sqlb);cl($db->error);}
              $rowb=$resultb->fetch_assoc();
			  ...
			  //if ((($knowledgelevel+$softlevel) > 2) && ($level>1)) {
				...
				if ($level>9) {
					if ($rank>0) {
					echo "<img src='ranks/rank$rank.png' width=32 height=32>";
				}
				}
				echo "</td>";
                echo "<td>";
      ...
      elseif($a=='trends'){
		...
		while($row=$result->fetch_assoc()){
          ...
          if (($c2!='') && ($m2!='')){
            ...
            $qsql = "SELECT distinct $c2 FROM ahtreport_data WHERE month = '$monti' $ahtteamdefinition ORDER by $c2 ASC";
  				  $contract = 5;
  				  $teammate_counter=0;
            if(!$qresult=$db->query($qsql)){cl($qsql);cl($db->error);}
            $contract = 5;$x=0;
              ...
              if ($displayme) {
                ...
                for ($x=1;$x<13;$x++){
                  ...
                  if ($qsurveys > 0) {
                    ...
                    echo " style='background-color:#$bg;color:#$fg;font-size:".(6+($qsurveys/20)) . "pt;' title='$qsurveys'>";
      ...
      elseif($a=='dashboard') {
        ...
        echo "<tr><td></td>
          <td class='sectionheader'><a href='?a=team&b=5'>Danish</a><span class='sectioncounter' title='Surveys received'>$dksurveys</span>
          <div class=latest>".getlatest($teamdef['Denmark'])."</div></td>
          <td class='sectionheader'><a href='?a=team&b=7'>Dutch</a><span class='sectioncounter' title='Surveys received'>$nlsurveys</span>
          <div class=latest>".getlatest($teamdef['Netherlands'])."</div></td>
          <td class='sectionheader'><a href='?a=team&b=6'>Norwegian</a><span class='sectioncounter' title='Surveys received'>$nosurveys</span>
          <div class=latest>".getlatest($teamdef['Norway'])."</div></td>
          <td class='sectionheader'><a href='?a=team&b=4'>Swedish</a><span class='sectioncounter' title='Surveys received'>$sesurveys</span>
          <div class=latest>".getlatest($teamdef['Sweden'])."</div></td>
        </tr>";
